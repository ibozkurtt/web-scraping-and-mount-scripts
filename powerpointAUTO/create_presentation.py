...
def create_presentation(input_images_folder, output_images_folder, save_path):
    ...
    for input_image_name in input_images:
        ...
        if os.path.exists(output_folder_path):
            ...
            top_output = Inches(2.5)  # Starting position for the top
            ...
            for i, output_image_name in enumerate(output_images):
                ...
                resize_image(os.path.join(output_folder_path, output_image_name), resized_output_img_path, max_width=Inches(5.08), max_height=Inches(5.715))  # Resize output images
                pic_output = slide.shapes.add_picture(resized_output_img_path, left_output, top_output, width=Inches(2), height=Inches(2.5))
                
                # Move to the next row after every five images
                if (i + 1) % 5 == 0:
                    left_output = Inches(0)
                    top_output += Inches(2.5)  # 1.143 inches = 2.9 cm (1 inch = 2.54 cm)
                else:
